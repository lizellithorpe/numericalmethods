...
mercury.assignOrb(0.387,0.205,7.004*np.pi/180,np.random.random_sample()*2*np.pi,np.random.random_sample()*2*np.pi,np.random.random_sample()*2*np.pi)
mercury.assignCart()
...
venus.assignOrb(0.723,0.006,3.394*np.pi/180,np.random.random_sample()*2*np.pi,np.random.random_sample()*2*np.pi,np.random.random_sample()*2*np.pi)
venus.assignCart()

txtfile[:][1] = np.array([venus.x,venus.y,venus.z,venus.vx,venus.vy,venus.vz,venus.mass])
earth = MassiveBody('earth',3.003e-6)
earth.assignOrb(1,0.016,0.00005*np.pi/180,np.random.random_sample()*2*np.pi,np.random.random_sample()*2*np.pi,np.random.random_sample()*2*np.pi)
earth.assignCart()
txtfile[:][2] = np.array([earth.x,earth.y,earth.z,earth.vx,earth.vy,earth.vz,earth.mass])
mars = MassiveBody('mars',3.23e-7)
mars.assignOrb(1.52,0.093,1.85*np.pi/180,np.random.random_sample()*2*np.pi,np.random.random_sample()*2*np.pi,np.random.random_sample()*2*np.pi)
mars.assignCart()
txtfile[:][3] = np.array([mars.x,mars.y,mars.z,mars.vx,mars.vy,mars.vz,mars.mass])
jupiter = MassiveBody('jupiter',2.96e-4*0.0009543)
jupiter.assignOrb(5.203,0.048,1.305*np.pi/180,np.random.random_sample()*2*np.pi,np.random.random_sample()*2*np.pi,np.random.random_sample()*2*np.pi)
jupiter.assignCart()
